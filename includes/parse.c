...
/*   Updated: 2023/08/08 18:51:53 by tyamauch         ###   ########.fr       */
/* ************************************************************************** */

/*                                                                            */
#include "parse.h"

t_ast *parse(t_token **token_adress, t_data *d) 
{
...
{
	t_ast *node;
	node = (t_ast*)try_calloc(1,sizeof(t_ast));
	node ->type = command;
...
t_command *ast_init_word_node(t_command *node,t_token *token)
{
...
t_word ast_init_redirect_word_node(t_redirect *node);
{
	node = try_calloc(1,sizeof(t_redirect));
	set_word(node->word);
	set_redirect_word_type(token);//redirectタイプをsetする関数
}
