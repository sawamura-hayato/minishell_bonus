...
void expand_token_redirect_list(t_redirect_list_list *token,
								t_data *d,
								t_redirect_list_type is_quote);
void expand_variable_word_list(t_word_list **head, t_data *d);
void expand_variable_redirect_list(t_redirect_list_list **head, t_data *d);
...
// delete_redirect.c
void redirect_free(t_redirect_list_list *redirect);
void redirect_delete_head(t_redirect_list_list **head);
void redirect_delete_tail(t_redirect_list_list **head);
void redirect_delete_target(t_redirect_list_list **head, t_redirect_list_list *target);
...
//  void	expand_splitting_redirect_list(t_redirect_list_list *redirect_list, t_envs *ifs);
//  void	expand_word_splitting(t_ast *node, t_data *d);

// expand_filename.c
void expand_filename(t_ast *node);
void expand_filename_word_list(t_word_list *word_list);
void expand_filename_redirect_list(t_redirect_list_list redirect_list);
...
void expandion_delete_quotation_redirect_list(t_redirect_list_list *redirect_list);

// expand_debug.c
t_ast *debug_new_ast(t_command *command, t_ast_node_type type);
t_command *debug_new_command(t_word_list *word_list, t_redirect_list_list *redirect_list);
t_word_list *debug_new_word_list(char *word, size_t index, t_token_type type);
t_redirect_list_list *debug_new_redirect_list(char *word, size_t index, t_redirect_list_type type);

void debug_printf_word_list(t_word_list *word_list);
void debug_free_word_list(t_word_list *word_list);
void debug_printf_redirect(t_redirect_list_list *redirect);
void debug_free_redirect(t_redirect_list_list *redirect);
#endif
