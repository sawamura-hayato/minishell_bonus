...
/*   Updated: 2023/09/06 18:34:32 by hsawamur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void expand_splitting_word_list(t_word_list *word_list, t_envs *ifs)
{
	...
	if (expand_is_ifs_default(ifs->value) || ft_strchr(ifs->value, ' '))
	{
		...
		{
			if (word_list->type == SINGLE_QUOTE || word_list->type == DOUBLE_QUOTE)
			{
				word_list = word_list->next;
				while (word_list->type == SINGLE_QUOTE || word_list->type == DOUBLE_QUOTE)
					word_list = word_list->next;
			}
